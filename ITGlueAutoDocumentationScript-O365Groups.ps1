...
$FlexAssetName = "O365 Groups"
$Description = "Documentation of O365 groups and settings."
...
Write-Host "Grabbing all Teams" -ForegroundColor Green
    ...
    foreach ($group in $Groups) {
        $Headers2 = Get-ImmyAzureAuthHeader
        $Owners = (Invoke-RestMethod -Uri "https://graph.microsoft.com/beta/groups/$($group.id)/owners" -Headers $Headers2 -Method Get -ContentType "application/json").value
        $Members = (Invoke-RestMethod -Uri "https://graph.microsoft.com/beta/groups/$($group.id)/members" -Headers $Headers2 -Method Get -ContentType "application/json").value
        ...
        $TaggedMembers = foreach ($Member in $Members) {
            ...
            if ($email) {
                ...
                $AllITGlueContacts | Where {$_.attributes.'contact-emails'.value -eq $email} | select -first 1
                start-sleep 1
            }
        }
    ...
    write-host "   Uploading $($group.displayName) into IT-Glue"



    foreach ($org in $orgID | Select-Object -Unique) {
        $ExistingFlexAsset = $AllTeamsAssets | Where-Object { $_.attributes.traits.'group-id' -eq $FlexAssetBody.attributes.traits.'group-id' }
